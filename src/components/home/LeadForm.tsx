...
const destinations = [
  { value: '', label: 'בחרו יעד' },
  { value: 'ayia-napa', label: 'איה נאפה' },
  { value: 'malia', label: 'מאליה' },
  { value: 'rhodes', label: 'רודוס' },
  { value: 'sunny-beach', label: 'סאני ביץ׳' },
  { value: 'lloret-de-mar', label: 'יורט דה מאר' },
...
export default function LeadForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      setTimeout(() => {
        const destLabel = destinations.find(d => d.value === data.destination)?.label || '';
        const message = `היי, אני ${data.name}, גיל ${data.age}. אשמח לשמוע על חופשות ל${destLabel}!`;
  ...
  const openWhatsApp = () => {
    ...
    const message = data.name
      ? `היי, אני ${data.name}${data.age ? `, גיל ${data.age}` : ''}. אשמח לשמוע על חופשות${destLabel ? ` ל${destLabel}` : ''}!`
      : 'היי, אשמח לשמוע על חופשות!';
    window.open(`https://wa.me/972515699420?text=${encodeURIComponent(message)}`, '_blank');
  };
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-2xl shadow-xl p-8">
              <div className="space-y-5">
                ...
                <div>
                  ...
                  <select
                    {...register('destination', { required: 'בחרו יעד' })}
                    className={`w-full px-4 py-3 rounded-xl border ${
              ...
              <div className="mt-8 space-y-3">
                ...
                >
                  {isSubmitting ? 'שולח...' : 'תמצאו לי חופשה'}
                </button>
